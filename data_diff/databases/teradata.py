...
class TeradataDialect(BaseDialect):
    ...
    def md5_as_int(self, s: str) -> str:
        # TO_NUMBER(substr(HASH_MD5('abc'), 1+32-12), 'xxxxxxxxxxxxxxx') - 140737488355327
        # return f"('x' || substring(md5({s}), {1+MD5_HEXDIGITS-CHECKSUM_HEXDIGITS}))::bit({_CHECKSUM_BITSIZE})::bigint - {CHECKSUM_OFFSET}"
        return f"to_number(substr(hash_md5({s}), {1+MD5_HEXDIGITS-CHECKSUM_HEXDIGITS}), 'xxxxxxxxxxxxxxx') - {CHECKSUM_OFFSET}"
...
class Teradata(ThreadedDatabase):
    DIALECT_CLASS: ClassVar[Type[BaseDialect]] = TeradataDialect
    SUPPORTS_UNIQUE_CONSTAINT = True
    CONNECT_URI_HELP = "teradata://<user>:<password>@<host>/<database>"
    ...
    def create_connection(self):
        td = import_teradata()
        try:
            self._conn = td.connect(**self._args)
            return self._conn
        except Exception as e:
            raise ConnectError(*e.args) from e

    def select_table_schema(self, path: DbPath) -> str:
        database, table = self._normalize_table_path(path)

        info_schema_path = ["dbc", "columns"]
        ...
        return f"""SELECT column_name, data_type, datetime_precision,
                    ...
                    WHERE tablename = '{table}' AND databasename = '{database}'
            """
    ...
    def _normalize_table_path(self, path: DbPath) -> DbPath:
        ...
        raise ValueError(
            f"{self.name}: Bad table path for {self}: '{'.'.join(path)}'. Expected format: database.table"
        )
